...
$(document).ready(function() {
	...
	if(wd_width <= 992) {
		...
		if ($('.home-hero-row .vertical-wrapper').length) {
			$('.home-hero-row .vertical-wrapper').show();
		} else {
		...
		$('#menuHeading').unbind('click touchstart').on('click touchstart', function(e){
			e.preventDefault();
			if ($(this).closest('.home-hero-row').length) {
				return;
			}
	...
	$(window).resize(function() {
		...
		if(sp_width <= 992){
			...
			if ($('.home-hero-row .vertical-wrapper').length) {
				$('.home-hero-row .vertical-wrapper').show();
				$('div.vertical-wrapper').not('.home-hero-row .vertical-wrapper').hide();
			} else {
				$('div.vertical-wrapper').hide();
			}
			$( "ul.megamenu > li.hover").unbind( "click" );
...
function hoverMegaMenu(){
		$("ul.megamenu > li.hover").hover(function () {
			...
			var transition = $(this).closest(".megamenu").data("transition");
			var animation_time = $(this).closest(".megamenu").data("animationtime");
			var isHeroMenu = $(this).closest('.home-hero-row').length > 0;
			$("ul.megamenu > li").removeClass("active");
			$(this).addClass("active");
			$("ul.megamenu > li").children(".sub-menu").hide();
			$("ul.megamenu > li").find(".content").hide();
			$(this).children(".sub-menu").show();
			if (isHeroMenu) {
				$(this).find(".content").stop(true, true).show().css({ height: "100%", display: "block" });
				$(this).children(".sub-menu").css({ right: "auto", top: "0", left: "100%" });
				return;
			}
			if(transition == 'slide') {
				$(this).find(".content").show();
